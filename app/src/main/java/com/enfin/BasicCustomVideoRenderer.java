    ...
    static class MyRenderer implements GLSurfaceView.Renderer {
        ...
        public void displayFrame(Frame frame) {
            ...
            if(mCustomVideoRenderer.mSaveScreenshot) {
                ...
                int half_width = (width + 1) >> 1;
                int half_height = (height[0] +1) >> 1;
                int y_size = width * height[0];
                int uv_size = half_width * half_height;

                byte []yuv = new byte[y_size + uv_size * 2];
                bb.get(yuv);
                int[] intArray = new int[width* height[0]];

                // Decode Yuv data to integer array
                decodeYUV420(intArray, yuv, width, height[0]);

                // Initialize the bitmap, with the replaced color
                Bitmap bmp = Bitmap.createBitmap(intArray, width, height[0], Bitmap.Config.ARGB_8888);
                ...
                try {
                   ...
                   // fOutputStream.flush();
                    ...
                    int width1 = bmp.getWidth(), height1 = bmp.getHeight();
                    int[] pixels = new int[width1 * height1];
                    bmp.getPixels(pixels, 0, width1, 0, 0, width1, height1);
                    bmp.recycle();
                    bmp = null;
                    RGBLuminanceSource source = new RGBLuminanceSource(width1, height1, pixels);
                    BinaryBitmap bBitmap = new BinaryBitmap(new HybridBinarizer(source));
                    MultiFormatReader reader = new MultiFormatReader();
                    try
                    {
                        Result result = reader.decode(bBitmap);
                        Toast.makeText(mContext,"Result : "+result.getText(), Toast.LENGTH_SHORT).show();
                    }
                    catch (NotFoundException e)
                    {
                        Log.e("decode exception", e.getMessage());
                    }
